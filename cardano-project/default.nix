      ...
      import ((import ./.obelisk/impl {}).path + "/dep/reflex-platform")
        (args // {
          ...
          nixpkgsOverlays = let deps = rpSetup.nixpkgs.thunkSet ./cardano-overlays/dep; in (args.nixpkgsOverlays or []) ++ [
            # Ensure all builder environments have access to nix-prefetch-* helpers by default.
            # This augments stdenv's initial PATH so derivations like cabal2nix-*.drv can find them
            # even if the tool isn't listed explicitly as a nativeBuildInput.
            (self: super: {
              stdenv = super.stdenv // {
                # Prefer script-based nix-prefetch-url from nix-prefetch-scripts to avoid arch-specific binary issues
                initialPath = (super.stdenv.initialPath or []) ++ [
            ...
            (self: super: {
              ...
              cabal2nix = super.runCommand "cabal2nix-with-prefetch-shim" { } ''
                ...
                #!/usr/bin/env bash
                set -euo pipefail
                export PATH="${super.nix}/bin:${super.coreutils}/bin:${super.findutils}/bin:${super.gnused}/bin:${super.gnugrep}/bin:${super.gnutar}/bin:${super.xz}/bin:${super.gzip}/bin:${super.bzip2}/bin:${super.curl.bin}/bin:$PATH"
                exec ${super.nix-prefetch-scripts}/bin/nix-prefetch-url "$@"
                EOS
                chmod +x $out/bin/nix-prefetch-url
              '';
              # Also override the tool in buildPackages since callCabal2nix uses buildPackages.cabal2nix
...
cardanoProjectDef = ({ nixpkgs, pkgs, hackGet, ... }@args: let
  ...
  userSpaceOverrides = (self: super: {
    ...
    cardano-crypto-class = haskellLib.overrideCabal (self.callCabal2nix "cardano-crypto-class" (deps'.cardano-base + "/cardano-crypto-class") { libsodium = pkgs.libsodium-vrf; libblst = null; crypton = null; }) (drv: {
